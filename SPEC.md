...
- ✅ **2026-05-29 安静自然氛围背景**：用户希望打开即"安静下来"。落地为**纯 CSS 渐变天光 + 自绘 SVG 远山剪影**，按本地时间切换晨/昼/暮/夜四套配色（可在右上角手动锁定或"跟随时间"，存 storage）；叠加极缓慢的光晕呼吸、薄雾横移、远山视差三层动效（`prefers-reduced-motion` 下冻结）。**零图片、零网络、零版权风险**，符合 §3。
- ✅ **2026-05-29 速记区改用 CodeMirror 6（Markdown）**：原生 textarea 升级为 CM6 markdown 编辑器（最小化配置：无行号/工具栏，仅高亮+折行+撤销+Tab，主题走 CSS 变量跟随四时段）。**这是对 §6"零运行时依赖"的有意放宽**——bundle 由 ~18KB 增至 ~250KB(gzip)，新增 6 个 `@codemirror/*` 依赖。理由：用户场景是 vibe coding 随手记，Markdown + 代码块价值大。仍满足"不用 React/Vue 框架"。速记的 sessionStorage 临时性（§3 第 3 条）保持不变。MV3 禁远程代码，故必须打包进 bundle（不可 CDN）。
- ✅ **2026-05-29 鼓励语（每日一句的本地化实现）**：日期下方加一句鼓励的话，每次打开页面随机刷新、点击可换一句。**用本地内置语录而非 P1 计划的 hitokoto API**——理由：本地优先/无网络（§3 第 4 条）、可精选"安静专注"的语气、MV3 无需远程权限、离线可用、零版权。原 §4 P1"每日一句（hitokoto）"由此调整为本地实现。
- ✅ **2026-05-31 音乐槽扩展到 6 个变体**：原"轻音乐"槽从单一文件扩到 6 个变体（`music-1.mp3` ~ `music-6.mp3`），UI 复用既有 variant 选择器（点击小圆点切换）。素材来自用户本地 `/Users/joseph/Workspace/Studio/Music`，文件名按字母序映射为 1~6。**两个待跟进项**：(1) 6 个文件协议未确认，`SOURCES.md` 已标 TODO，分发前需对齐 §3 第 6 条"零版权风险"；(2) 单文件 ~1.7–2MB，超过 §4 "音乐 ≤ 1.5MB" 预算，需 ffmpeg 重压。`noise.js` 的 `DEFAULT_PRESET.music.variant` 仍为 1，`welcome.js` 的 `WELCOME_PRESET.music.variant` 仍为 1，用户已存储的偏好不受影响。
